...
from typing import Tuple, Callable, List, Optional, Type, Union, Dict, cast

import torch
import torch.nn as nn
from torch import Tensor
from transformers import ViTModel, ViTConfig
from transformers.models.vit.modeling_vit import ViTEncoder, ViTPooler
...
class URESNET18(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv1d(c_in, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
...
class GRU(nn.Module):
    def __init__(self, c_in, gru_units, num_classes):
        super().__init__()
...
class VGG(nn.Module):
    def __init__(
        self, c_in, num_classes: int = 1000, init_weights: bool = True, dropout: float = 0.5
    ) -> None:
        super().__init__()
        self.features = make_layers(cfgs["A"], batch_norm=False, in_channels=c_in)
...
class ViT(ViTModel):
    def __init__(self, num_classes, config: ViTConfig, add_pooling_layer: bool = True, use_mask_token: bool = False):
    ...
    def forward(
        ...
        # )
        embedding_output = x.transpose(1, 2) # b, c, t -> b t c
